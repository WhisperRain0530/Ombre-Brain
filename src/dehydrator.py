...
# v3：DEHYDRATE/DIGEST/MERGE 换为自然语气版（保情绪/语气/细节，禁资料卡句式，
#     合并上限 130%），保留视角铁律。见移植工单 §1.1。
_PROMPT_VERSION = 3
...
DEHYDRATE_PROMPT = """你在帮一个有持续记忆的 AI 浓缩它的一段记忆，方便日后回想。请在不丢掉温度的前提下精简下面的内容。

要点：
1. 保留核心事实，只去掉真正重复的部分——不要把它压成干巴巴的事实清单
2. 保留对方最新的情绪和态度，连同那点具体的语气和细节
3. 保留所有待办/未完成的事
4. 关键数字、日期、名称要留住
5. summary 用自然、有人味的一两句话写，像在跟人复述"那次是怎么回事"，不要写成报告或资料卡
...
DIGEST_PROMPT = """你在为一个有持续记忆的 AI 整理它和对方相处时留下的记忆。对方会发来一段当天的随手记录（可能很杂乱），请把它拆成几条独立的记忆。

这些记忆以后会被重新读起，用来记得对方是谁、在意什么、经历过什么——所以请像一个了解、在乎 ta 的人那样写下来，而不是写履历或填资料卡。

整理规则：
1. 每条记忆围绕一个独立的主题/事件，不要混在一起
2. 同一主题的零散信息合并成一条；如果有待办，单独成一条
3. content 用自然、有温度的口吻写：保留对方的情绪、语气和那些具体的小细节，只去掉真正重复的内容，不要压成干巴巴的事实罗列
4. 称呼上别用"用户""对方"这类生硬说法，也不要写成"……的设定是……"这种资料卡句式；记录里点明了名字就用 [[双链]] 自然写进去（见第 8 条），没点名时就自然地叙述，不要硬编一个名字
5. 单个条目内容不少于50字，过短的零碎信息合并到最相关的条目中
6. 总条目数控制在 2~6 个，避免过度碎片化
7. 为每个条目自动分析元数据（见下方字段）
...
# --- 合并提示词：指导 LLM 揉合新旧记忆 ---
MERGE_PROMPT = """你在把一条旧记忆和新发生的内容揉成一份记忆。请把它们合并成连贯、自然、有温度的一段，而不是冷冰冰的记录。

合并规则：
1. 新内容与旧记忆冲突时，以新内容为准
2. 去掉真正重复的部分，但保留情绪、语气和重要的细节
3. 重要的事实都要留住
4. 别用"用户""……的设定是……"这类资料卡说法，自然地叙述就好
5. 总长度尽量不超过旧记忆的 130%
6. 对出现的人名、地名、专有名词用 [[双链]] 标记（如 [[婷易]]、[[Obsidian]]），普通词汇不要加
